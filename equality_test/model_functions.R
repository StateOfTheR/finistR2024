...
# I want as parameters n, mu1, mu2, sigma and 1 simulation is a couple of sequences.
make_my_model_normal <- function(n, mu1,mu2, sig) {
  new_model(name = "normal",
            ...
            simulate = function(n, mu1,mu2, sig, nsim) {
              ...
              li1=split(x1, col(x1))
              li2=split(x2, col(x2))
              li = lapply(1:nsim, function(i){cbind(li1[[i]],li2[[i]])})
              return(li) # make each col its own list element
            })
}
